...
Python port of the `burden` R package: sample-size-adjusted differential
expression (DE) "burden" for single-cell data. Cell types with more cells
tend to show more significant DE genes for reasons of statistical power
alone, which biases naive DEG counts toward larger cell types. `burden`
downsamples each cell type across a range of cell counts, refits DE at each,
and models DEG count as a function of cell count — so you can ask "how many
DEGs would this cell type show if it had as many cells as a typical cell
type in this dataset," rather than comparing raw DEG counts across cell
types of very different size.
...
pip install -e "path/to/burden/python"

# optional DE backends
pip install -e "path/to/burden/python[edger]"    # requires rpy2 + R + edgeR
pip install -e "path/to/burden/python[deseq2]"   # requires pydeseq2
```
...
`de_fun` can be any callable following the contract in `burden.de_wrappers`:
`f(counts, group, **kwargs) -> pd.DataFrame(gene, pval, padj, logFC)`, so any
DE method (not just the three built-ins) can be plugged in.
...
- **Quasi-Poisson fitting.** R fits via `mgcv::gam(family = quasipoisson(link
  ...
  (`burden.model_fit.quasi_f_test`), the same statistic family as R's
  `anova(..., test="F")`.
